...
## Melhorias recomendadas (para nota 8+)

1. Adicionar seção de arquitetura no `README.md` com resumo curto e link para este arquivo.  
2. Dashboard: adicionar GMV por loja, bottom performers, margem bruta e um gráfico de sensibilidade do desconto sobre lucro.  
3. Processador: adicionar batching, idempotência (evitar duplicados) e métricas.  
4. Pipeline de testes: incluir script local que gera X eventos e verifica consistência das agregações no DuckDB.

---

Arquivo criado automaticamente para complementar o README do trabalho. Para mudanças rápidas no conteúdo, edite este arquivo em `trabalho/ARCHITECTURE.md`.
